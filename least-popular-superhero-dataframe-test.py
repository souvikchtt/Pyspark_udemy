...
movie_id_df4=movie_id_df3.sort(f.col("appearence_count"))
...
#print(result)
join_result_test=movie_names_df.join(movie_id_df4,on="id", how="inner").sort(f.col("appearence_count")).show(5)
join_result=movie_names_df.join(movie_id_df4,on="id", how="inner").sort(f.col("appearence_count")).first()

print("The least popular superhero is "+join_result[1]+" having id: "+str(join_result[0])+" and appear count is "+str(join_result[2]))
